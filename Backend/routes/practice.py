...
def get_topics(student_id: int, db: Session = Depends(get_db)):
    ...
    try:
        from classroom_models import StudentClass, ClassCurriculum, Classroom
...
async def _get_practice_data_impl(student_id: Optional[int] = None, class_id: Optional[int] = None, db: Session = Depends(get_db)):
    ...
    if student_id:
        ...
        # Call this first as it synchronizes and potentially purges TopicPerformance rows
    ...
    # ── PRIORITY 3: Curriculum (Class Curriculum OR Default) ──
    if class_id:
    ...
    if student_id:
        ...
        session_stats = {
            ...
            "time": f"{questions_attempted * 2}m",  # rough estimate
