...
def _daily_topic_tiebreaker(student_id: int, topic: dict) -> str:
    """Provide a stable per-student, per-day order for equally urgent topics."""
    today = datetime.utcnow().date().isoformat()
    subject = str(topic.get("subject", "")).strip().casefold()
    name = str(topic.get("topic", "")).strip().casefold()
    value = f"{student_id}:{today}:{subject}:{name}".encode("utf-8")
...
def generate_study_plan(
    ...
    sorted_plan = sorted(
        ...
        key=lambda x: (-x["priority_score"], _daily_topic_tiebreaker(student_id, x)),
    )
